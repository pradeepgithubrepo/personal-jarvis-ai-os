...
We use `pypdf` as our PDF reader engine. The `parse_pdf(file_bytes)` function in [pdf_parser.py](file:///home/prad/petprojects/ai/jarvis/src/agents/consumer/parsers/pdf_parser.py) extracts raw text from each page, yielding a structured `ParsedDocument` containing raw string text, page lists, and parsed tables.
...
* **Multi-Line Description:**
  Subsequent lines are captured as part of the description block until the amount/balance line is hit.
...
* **Amount & Balance Extraction:**
  - **Debit Pattern:**
    ...
    ```
    Matches `- [debit_amt] - [balance]`.
  - **Credit Pattern:**
    ```text
    ^-\s+-\s+([\d,]+\.\d{2})\s+([\d,]+\.\d{2})\s*(?:CR)?$
    ```
    Matches `- - [credit_amt] [balance]`.
...
* **Row Pattern:**
  ...
  ```
  Extracts Date, Narration, Value Date, Amount, and Balance.
